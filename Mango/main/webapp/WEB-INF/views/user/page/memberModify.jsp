...
<body>
   ...
   <div class="wrapper">
      ...
      <div class="inner">
         ...
         <form action="/page/memberModify/update.do" method="post" id="modifyForm">
            ...
            <div class="form-wrapper">
               <input name="name" type="text" id="name" placeholder="이름"
                  class="form-control" value="${login.getName()}"> <i class="zmdi zmdi-account-box"></i>
            </div>

            <div class="form-wrapper phone_num">
               <input type="text" name="phone" placeholder="휴대폰번호"
                  class="form-control" id="phone" value="${login.getPhone()}" readonly> <i class="zmdi  zmdi-phone"></i>
               <button id="ph" class="pnCheckButton" type="button" onClick="opensms();">문자인증</button>
            </div>

            <div class="form-wrapper">
               <input type="email" name="email" placeholder="이메일"
                  class="form-control" value="${login.getEmail()}"> <i class="zmdi zmdi-email"></i>
            </div>
            
            <div class="form-wrapper">
              <input class="form-control" type="text" placeholder="우편번호" style="width:315px;" id="zipNo"  name="zipNo" readonly />
              <button id="jusoCheck" type="button" value="${login.getZipNo()}" onclick="goPopup();">주소검색</button>
              <i class="zmdi zmdi-phone"></i>
            </div>
            <div class="form-wrapper">
              <input class="form-control" type="text" placeholder="도로명주소" style="width:315px;" id="address"  name="address" readonly/>              
              <i class="zmdi zmdi-phone" value="${login.getAddress()}"></i>
            </div>
            <button type="button" type="submit" onclick="checks();">회원정보 수정</button>
            <button type="button" type="submit" onclick="return submit2(this.form);">회원탈퇴</button>
         </form>
      </div>
   </div>
