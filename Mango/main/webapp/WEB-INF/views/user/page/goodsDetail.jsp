...
<body>
	...
	<form id="cart_direct" action="/page/cart/insert.do" method="post">
		...
		<input class="id_submit" type="hidden" name="id"
			value="<%=login.getId()%>"> <input class="goods_qty_submit"
			type="hidden" name="goodsQty" value="1"> <input
			class="goods_delivery_submit" type="hidden" name="deliveryPrice"
			value="${goods.deliveryPrice}"> <input class="seller_submit"
			type="hidden" name="sellerName" value="${goods.sellerName}">
		<input class="goods_name_submit" type="hidden" name="goodsName"
			value="${goods.goodsName}"> <input
			class="goods_category_submit" type="hidden" name="goodsCategory"
			value="${goods.goodsCategory}">
	</form>

	<!-- orders로 값 전달 -->
	<form id="orders_direct" action="directOrders/insert.do" method="post">
		<input class="id_submit" type="hidden" name="id"
			value="<%=login.getId()%>"> <input class="sellerName_submit"
			type="hidden" name="sellerName" value="${goods.sellerName}">
		<input class="name_submit" type="hidden" name="name"
			value="<%=login.getName()%>"> <input
			class="goods_category_submit" type="hidden" name="goodsCategory"
			value="${goods.goodsCategory}"> <input
			class="goods_name_submit" type="hidden" name="goodsName"
			value="${goods.goodsName}"> <input class="goods_stock_submit"
			type="hidden" name="goodsStock" value="${goods.goodsStock}">
		<input class="goods_img_submit" type="hidden" name="goodsImg"
			value="${goodsFileList[0].realPath}"> <input
			class="goods_delivery_submit" type="hidden" name="deliveryPrice"
			value="${goods.deliveryPrice}"> <input
